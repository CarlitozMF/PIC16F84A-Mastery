;*************************************************************************************************
; PROYECTO: Saltos_04 - Introduccion a las comparaciones
...
; Si VALOR_LEIDO > 16	 =>  C = 1 AND Z=0
; Si VALOR_LEIDO =<  16  =>  C = 0 OR Z=1
	BTFSC	STATUS,Z	;¿Z = 0?    Si Z=0, evalua C
	goto	MODO_ELSE	;Si Z = 1, no salto y goto manda a modo else
	
	BTFSS	STATUS,C	;¿C = 1? Si C=1 es mayor
	goto	MODO_ELSE	;Si C = 0, es menor y goto manda a modo else
	
MODO_IF				;Se ejecuta si es mayor estricto
	MOVLW	LED_ON_1
	MOVWF	PORTB
	goto	MAIN
MODO_ELSE			;Se ejecuta si es menor o igual
	MOVLW	LED_ON_2
	MOVWF	PORTB
	goto	MAIN
