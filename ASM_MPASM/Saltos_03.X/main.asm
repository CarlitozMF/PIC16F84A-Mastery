;*************************************************************************************************
; PROYECTO: Saltos_03 - Introduccion a las comparaciones
...
; Lógica del Carry (C):
; Si VALOR_LEIDO >= 16  =>  C = 1
; Si VALOR_LEIDO <  16  =>  C = 0
	BTFSC	STATUS,C	;¿C = 0?    Si C=0 salta a modo if
	goto	MODO_ELSE	;Si C = 1, no salto y goto manda a modo else	
	
MODO_IF				;Se ejecuta si C=0
	MOVLW	LED_ON_2
	MOVWF	PORTB
	goto	MAIN
MODO_ELSE			;Se ejecuta si C=1
	MOVLW	LED_ON_1
	MOVWF	PORTB
	goto	MAIN
